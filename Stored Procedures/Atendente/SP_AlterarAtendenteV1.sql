...
BEGIN
	...
	BEGIN			
		...
		ROLLBACK
	END
	ELSE IF NOT EXISTS (Select @ATSITATLOK From DB_LOKANDO..TBATNDLOK With(nolock) Where @ATSITATLOK = 'A' Or @ATSITATLOK = 'I' Or @ATSITATLOK = 'B')
	BEGIN
	...
	BEGIN
		...
		Update DB_LOKANDO..TBATNDLOK Set ATNOMELOK = @ATNOMELOK, ATSITATLOK = @ATSITATLOK, ATHRREG = GETDATE() Where ATIDATLOK = @ATIDATLOK;
		Update DB_LOKANDO..TBUSULOK Set USSITLOK = @ATSITATLOK, USUHRREG = GETDATE() Where USIDUSU = @IDUSUATEND;
